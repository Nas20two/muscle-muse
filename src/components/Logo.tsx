...
const sizeConfig = {
  sm: { icon: 32, iconMobile: 24, text: "text-lg sm:text-2xl" },
  md: { icon: 48, iconMobile: 32, text: "text-xl sm:text-3xl" },
  lg: { icon: 64, iconMobile: 48, text: "text-2xl sm:text-4xl" },
};

export default function Logo({ size = "md", showText = true, compactOnMobile = false }: LogoProps) {
  const config = sizeConfig[size];
  const iconSize = compactOnMobile ? config.iconMobile : config.icon;
  
  return (
    <div className={`flex items-center ${compactOnMobile ? 'gap-1.5 sm:gap-3' : 'gap-2 sm:gap-3'}`}>
      <div 
